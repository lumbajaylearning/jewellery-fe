...
export default function CheckoutPage() {
    ...
    return (
        <main className="mx-auto w-full max-w-7xl overflow-x-hidden px-4 py-8 sm:px-6 sm:py-10 lg:px-8 lg:py-16">
            ...
            <div className="mb-10">
                ...
                <h1 className="mt-3 font-heading text-3xl text-text-primary sm:text-5xl">Delivery and payment.</h1>
            ...
            <form onSubmit={handlePlaceOrder} className="grid min-w-0 gap-6 lg:grid-cols-[minmax(0,1fr)_380px] lg:items-start lg:gap-8">
                <div className="min-w-0 space-y-6">
                    <section className="min-w-0 overflow-hidden rounded border border-border bg-white p-4 sm:p-7">
                        ...
                        {savedAddresses.length > 0 && <div className="mt-6 min-w-0 max-w-full"><p className="text-xs font-semibold uppercase tracking-wider text-text-secondary">Saved addresses</p><div className="-mx-1 mt-3 flex max-w-full snap-x gap-3 overflow-x-auto px-1 pb-3">{savedAddresses.map((savedAddress) => <button key={savedAddress.id} type="button" onClick={() => selectSavedAddress(savedAddress)} className={`w-[76vw] max-w-64 flex-none snap-start rounded border p-3 text-left text-xs text-text-secondary ${selectedSavedAddressId === savedAddress.id && !addressFormOpen ? "border-gold bg-surface ring-1 ring-gold" : "border-border bg-white hover:border-gold"}`}><span className="flex items-center justify-between gap-2 font-semibold text-text-primary"><span className="truncate">{savedAddress.address_name || "Saved address"}</span>{selectedSavedAddressId === savedAddress.id && !addressFormOpen && <Check className="h-3.5 w-3.5 flex-none text-gold" />}</span><span className="mt-1 block truncate">{savedAddress.address_1}, {savedAddress.city}</span></button>)}</div></div>}
                        {!addressFormOpen && addressConfirmed && <div className="mt-5 min-w-0 rounded border border-gold bg-surface p-4"><div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-semibold text-text-primary">{address.first_name} {address.last_name}</p><p className="mt-1 break-words text-sm leading-6 text-text-secondary">{address.address_1}{address.address_2 ? `, ${address.address_2}` : ""}<br />{address.city}, {address.province} {address.postal_code}<br />{address.phone}</p></div><Check className="h-5 w-5 flex-none text-emerald-700" /></div><div className="mt-4 flex flex-wrap gap-4 border-t border-border pt-3"><button type="button" onClick={() => { setAddressConfirmed(false); setAddressFormOpen(true); }} className="text-xs font-semibold text-text-primary">Edit address</button><button type="button" onClick={useDifferentAddress} className="text-xs font-semibold text-gold">Use a different address</button></div></div>}
                        {addressFormOpen && <div className="mt-6 grid gap-4 sm:grid-cols-2">
                            ...
                            <div className="sm:col-span-2 flex flex-col-reverse gap-3 border-t border-border pt-4 sm:flex-row sm:flex-wrap sm:items-center sm:justify-end">
                                {savedAddresses.length > 0 && <button type="button" disabled={addressSaving} onClick={() => selectSavedAddress(savedAddresses.find((item) => item.id === selectedSavedAddressId) ?? savedAddresses.find((item) => item.is_default_shipping) ?? savedAddresses[0])} className="w-full px-3 py-2.5 text-xs font-semibold text-text-secondary disabled:opacity-50 sm:w-auto">Cancel</button>}
                                <button type="button" disabled={addressSaving} onClick={confirmAddressForOrder} className="w-full rounded border border-text-primary px-4 py-3 text-xs font-semibold text-text-primary disabled:opacity-50 sm:w-auto sm:py-2.5">Use for this order</button>
                                {customerAuthenticated && <button type="button" disabled={addressSaving} onClick={saveAddress} className="w-full rounded bg-text-primary px-4 py-3 text-xs font-semibold text-white disabled:opacity-50 sm:w-auto sm:py-2.5">{addressSaving ? "Saving…" : selectedSavedAddressId ? "Update saved address" : "Save and use address"}</button>}
                            </div>
                        </div>}
                    </section>

                    <section className="min-w-0 rounded border border-border bg-white p-4 sm:p-7">
                ...
                <aside className="min-w-0 rounded border border-border bg-surface p-4 sm:p-6 lg:sticky lg:top-28">
